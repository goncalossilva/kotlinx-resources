...
class ResourcesPlugin : KotlinCompilerPluginSupportPlugin {
    ...
    private fun setupWasmWasiResources(kotlinCompilation: KotlinCompilation<*>) {
        ...
        // Derive the executable compile task name. Unlike kotlinCompilation.compileKotlinTaskName
        // (which gives "compileTestKotlinWasmWasi"), we need the executable task that produces
        // .mjs files (e.g., "compileTestDevelopmentExecutableKotlinWasmWasi").
        val compileTaskName = getWasmExecutableTaskName(kotlinCompilation)
    ...
    /**
     * Derives the WASM executable compilation task name.
     *
     * For WASM targets, the Kotlin Gradle Plugin creates executable tasks with the pattern:
     * compile{CompilationName}{BuildType}ExecutableKotlin{TargetName}
     *
     * For example:
     * - compileTestDevelopmentExecutableKotlinWasmWasi
     * - compileTestProductionExecutableKotlinWasmWasi
     *
     * This differs from kotlinCompilation.compileKotlinTaskName which returns the basic
    ...
    private fun setupWasmWasiTestTask(
        ...
        val setupTask = tasks.register(taskName) { task ->
            ...
            // The .mjs file is generated by the executable compilation task,
            // which runs after the basic Kotlin compilation. We need to run after the .mjs exists.
